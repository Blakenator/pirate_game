Shader "Hidden/Crest/Underwater/Post Process HDRP"
{
	SubShader
	{
		Pass
		{
			...
			// #pragma shader_feature_local CREST_SUBSURFACESCATTERING_ON
			// #pragma shader_feature_local CREST_SUBSURFACESHALLOWCOLOUR_ON
			// #pragma shader_feature_local CREST_TRANSPARENCY_ON
			...
			#pragma shader_feature_local CREST_COMPILESHADERWITHDEBUGINFO_ON
			...
			half3 ApplyUnderwaterEffect(
				...
				{
					half sss = 0.0;
					const float3 uv_slice = WorldToUV(_WorldSpaceCameraPos.xz, oceanPosScale0, oceanParams0, sliceIndex0);
...
// #endif
					...
					(
						...
						// _IndirectLightingMultiplier allows control of baked lighting through volume framework. X is
						// indirect diffuse.
						_AmbientLighting * _IndirectDiffuseLightingMultiplier.x,
			...
			float4 Frag(Varyings input) : SV_Target
			{
				...
				// XY handles dynamic resolution and ZW handles screen size changes. We need to use these scale values
				// because RTHandles are never resized to be smaller.
				uvScreenSpace.xy /= _RTHandleScale.xy;
				uvScreenSpace.xy *= _RTHandleScale.zw;
				...
				// Comment-this back-in if the depth buffer no longer contains
				// the ocean depth. :OceanSurfaceDepthFudge
