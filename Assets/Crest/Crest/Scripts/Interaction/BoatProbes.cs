...
namespace Crest
{
    ...
    public class BoatProbes : FloatingObjectBase
    {
        ...
        {
            if (_forcePointsParent != null)
            {
                _forcePoints = new FloaterForcePoints[_forcePointsParent.childCount];
                for (int i = 0; i < _forcePointsParent.childCount; i++)
                {
                    _forcePoints[i] = new FloaterForcePoints
                    {
                        _offsetPosition = _forcePointsParent.GetChild(i).transform.localPosition,
                        _weight = 1f
                    };
                }
            }
    ...
    [CustomEditor(typeof(BoatProbes))]
    public class BoatProbesEditor : ValidatedEditor
    {
        public override void OnInspectorGUI()
        {
            base.OnInspectorGUI();

            var target = this.target as BoatProbes;

            if (GUILayout.Button("Prebuild probes"))
            {
                target.RecalculateProbes();
            }
        }
    }
}
